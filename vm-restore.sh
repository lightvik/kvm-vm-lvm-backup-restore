...
_vmexist(){
    echo `LANG=C virsh dominfo $1 2>/dev/null | grep -c UUID`
}
...
_vmisrun(){
    echo `LANG=C virsh domstate $1 | grep -c run`
}

_vmisshut(){
    echo `LANG=C virsh domstate $1 | grep -c shut`
}
...
_vmgetdisks(){
    echo `virsh domblklist $1 --details | awk '/disk/ { print $4}'`
}
